...
class Camera(models.Model):
    ...
    hall = models.ForeignKey(Hall, on_delete=models.RESTRICT, verbose_name=_("Toyxona"), db_index=False)
    ...
    roi = models.JSONField(null=True, blank=True, default=None, verbose_name=_("Zona (ROI)"), editable=False)
    screenshot = models.ImageField(
        upload_to="camera/screenshot", null=True, blank=True, default=None,
        verbose_name=_("Screenshot"), editable=False,
    )
    ...
    def type_total(self):
        ...
        total = {}
        for row in self.roi:
            typ = row.get("type", self.TYPE_ZONE)
            total[typ] = total.get(typ, 0) + 1
        return total

    class Meta:
        indexes = [
            models.Index(fields=["hall", "device_sn"])
        ]
